...
class MinibatchRlEvalWandb(MinibatchRlEval):
    ...
    def __init__(self, final_eval_only=False, eval_only=False, no_eval=False, probe_jumps=[], probe_condition=1,
        ...
        self.no_init_eval = no_init_eval
        self.eval_only = eval_only
        self.offline_model_save = offline_model_save
...
class OfflineEval(MinibatchRlEvalWandb):
    def __init__(self, epochs, dataloader, save_fn=None, start_itr=0, save_every=None, use_offline_data=True, *args, **kwargs):
        ...
        if self.algo.probe:
            ...
            self.algo.offline_ft_dataset = self.ft_dataloader

        if self.eval_only:
            self.dataloader = self.ft_dataloader
        else:
            self.dataloader = self.get_dataloaders(dataloader)[0]
    ...
    def get_dataloaders(self, args):
        return get_offline_dataloaders(**args)
    ...
    def train(self):
        ...
        if not self.no_init_eval:
            with logger.prefix(f"itr #0 "):
                ...
                eval_traj_infos, eval_time = self.evaluate_agent(0)
                self.log_diagnostics(0, eval_traj_infos, eval_time, ft_metrics=ft_metrics)

            if self.algo.probe and self.algo.supervised or self.eval_only:
                self.shutdown()
                return
        ...
        while not done:
            ...
            if self.itr == self.total_iters or self.itr % self.save_every == 0:
                ...
                self.log_diagnostics(self.itr, eval_traj_infos, eval_time, ft_metrics=ft_metrics)
