import torch
from kornia.augmentation import RandomAffine, RandomCrop, RandomResizedCrop, RandomHorizontalFlip
...
def find_weight_norm(parameters, norm_type=1.0) -> torch.Tensor:
    ...
    if norm_type == np.inf:
        ...
        total_norm = torch.norm(
            torch.stack([torch.norm(p.data.detach(), norm_type).to(device) for p in parameters]), norm_type
        )
    return total_norm
...
def update_state_dict_compat(osd, nsd):
    updated_osd = {
        k.replace("head.advantage", "head.goal_advantage")
        .replace("head.value", "head.goal_value")
        .replace("head.secondary_advantage_head", "head.rl_advantage")
        .replace("head.secondary_value_head", "head.rl_value"): v
...
def discount_return_n_step(reward, done, n_step, discount, return_dest=None, done_n_dest=None, do_truncated=False):
    ...
    if not do_truncated:
        rlen -= n_step - 1
    return_ = torch.zeros((rlen,) + reward.shape[1:], dtype=reward.dtype, device=reward.device)
    done_n = torch.zeros((rlen,) + reward.shape[1:], dtype=done.dtype, device=done.device)
