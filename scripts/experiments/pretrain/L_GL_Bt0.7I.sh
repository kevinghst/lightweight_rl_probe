...
python -m scripts.run \
  ...
  +offline.algo.spr_weight=0.002 \
  agent.model_kwargs.barlow_balance=0.7 \
  agent.model_kwargs.barlow_lambd=0.0051 \
  agent.model_kwargs.blocks_per_group=5 \
  agent.model_kwargs.cnn_scale_factor=1.5 \
  agent.model_kwargs.expand_ratio=4 \
  agent.model_kwargs.gru_input_size=600 \
  agent.model_kwargs.joint_embedding=True \
  agent.model_kwargs.kl_balance=0.95 \
  agent.model_kwargs.latent_dist_size=32 \
  agent.model_kwargs.latent_dists=32 \
  agent.model_kwargs.ssl_obj=barlow \
  agent.model_kwargs.transition_type=gru \
  agent.model_kwargs.use_ema=False \
  algo.batch_size=64 \
  algo.jumps=10 \
  algo.kl_weight=0.1 \
  ...
  offline.runner.no_init_eval=True \
